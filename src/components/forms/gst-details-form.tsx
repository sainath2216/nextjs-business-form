...
const gstTypes = ["Regular", "Composite Supplier", "Unregistered", "Consumer"] as const;
...
const formSchema = z.object({
  ...
  if (["Regular", "Composite Supplier"].includes(data.gstType) && !data.gstNumber) {
    return false;
  }
  return true;
}, {
  ...
  if (["Regular", "Composite Supplier"].includes(data.gstType) && !data.gstCertificate) {
    return false;
  }
  return true;
}, {
...
<FormField
  ...
  render={({ field }) => (
    <FormItem className="space-y-3">
      <FormLabel>Do you have MSME/Udyog Aadhaar No?*</FormLabel>
      <FormControl>
        <RadioGroup
...
{watchHasMsmeUdyog === "yes" && (
  <>
    <FormField
      ...
      render={({ field }) => (
        <FormItem>
          <FormLabel>MSME/Udyog Aadhaar No.*</FormLabel>
          <FormControl>
            <Input {...field} />
          </FormControl>
          <FormMessage />
    ...
    <FormField
      ...
      render={({ field }) => (
        <FormItem>
          <FormLabel>MSME/Udyog Aadhaar Certificate*</FormLabel>
          <FormControl>
            <Input
              type="file"
              accept=".pdf,.jpg,.jpeg,.png"
              onChange={(e) => handleFileUpload(e, "msmeUdyogCertificate")}
              className="cursor-pointer"
            />
          </FormControl>
          <FormMessage />
